...
- 直接控制副屏音量，按相对比例同步两块屏幕，或在联动时保持手动设定的音量平衡
- 可选自动跟随由系统或其他应用引起的主屏音量变化
- 可选择是否使用实体键切换模式，并配置按键行为和参与轮换的模式
- 实时显示主屏与副屏音量
...
| 双屏联动 | 调节主屏媒体音量，同时按照当前百分比将副屏映射到对应档位；也可选择自动同步由系统或其他应用引起的主屏音量变化。在设置中将联动方式改为“保持音量平衡”后，两块屏幕会一起增减并保留手动调出的音量差，而不是被重新匹配。 |
| 跟随焦点 | 根据最近交互的屏幕决定控制目标：上屏使用主屏音量，下屏使用副屏音量。按住音量键期间目标保持不变，避免连续调节时在两块屏幕间跳转。 |

四种模式都支持短按和长按连续调节。**设置 → 按键与控制**中的“调节步长”用于应用接管的实体音量键操作，包括副屏、双屏联动，以及跟随焦点指向下屏时的调节；主屏和跟随焦点指向上屏时仍采用 Android 原生步长。若启用实体模式键，可通过长按该键在**设置 → 按键与控制 → 参与轮换的模式**中勾选的模式（至少两种）之间依次切换；关闭后仍可在应用首页直接选择。
